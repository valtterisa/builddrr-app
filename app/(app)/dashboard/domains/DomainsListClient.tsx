...
export default function DomainsListClient() {
  ...
  const fetchProjectDomains = async (projectId: string) => {
    try {
      const response = await fetch(
        `/api/manage-domains?projectId=${encodeURIComponent(projectId)}`
      );
      if (!response.ok) {
        // Only log the error, don't show toast for missing domains
        console.warn(
          `No domains found for project ${projectId} or API error:`,
          response.status
        );
        return { domains: [] };
      }
      ...
      console.warn(
        "Network error while fetching domains for project:",
        projectId,
        error
      );
      return { domains: [] };
  ...
  const addDomainToProject = async (
    ...
    try {
      const response = await fetch("/api/(vercel)/manage-domains", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ projectId, domain, websiteId }),
      });
      if (!response.ok) {
  ...
  const removeDomainFromProject = async (
    projectId: string,
    domain: string,
    websiteId: string
  ) => {
    try {
      const response = await fetch(
        `/api/(vercel)/manage-domains?projectId=${encodeURIComponent(projectId)}&domain=${encodeURIComponent(domain)}&websiteId=${encodeURIComponent(websiteId)}`,
        {
          method: "DELETE",
        }
      );
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        // Fetch real websites and domains from database
        const response = await fetch("/api/user-domains?includeWebsites=true");

        if (response.ok) {
          const { domains: userDomains, websites: userWebsites } =
            await response.json();

          // Set websites
          setWebsites(userWebsites || []);

          // Set domains if user has pro plan or higher
          if (plan !== "starter") {
          ...
          const mockWebsites = [
            {
              ...
              projectId: "prj_example1",
            },
            {
              id: "2",
              name: "Tech Solutions Inc",
              url: `https://builddrr.app/tech-solutions`,
              projectId: "prj_example2",
            },
            {
              id: "3",
              name: "Coffee Shop",
              url: `https://builddrr.app/coffee-shop`,
              projectId: "prj_example3",
            },
          ];

          setWebsites(mockWebsites);

          // Fetch domains from Vercel directly for each project as fallback
          if (plan !== "starter") {
            ...
            for (const website of mockWebsites) {
              try {
                ...
                console.error(
                  `Failed to fetch domains for ${website.name}:`,
                  error
                );
              }
            }
  ...
  const handleDeleteDomain = async (domainId: string) => {
    ...
    const success = await removeDomainFromProject(
      website.projectId,
      domain.name,
      domain.websiteId
    );
    ...
    if (success) {
      ...
      toast({
        ...
        description:
          "Your domain has been successfully removed from both Vercel and your account.",
      });
    }
  };
  ...
  const handleAddDomainSubmit = async () => {
    ...
    const result = await addDomainToProject(
      website.projectId,
      newDomain,
      selectedWebsite
    );
    ...
    if (result) {
      ...
      const newDomainObj: Domain = {
        id: result.supabaseDomain?.id || `${selectedWebsite}-${newDomain}`,
        name: newDomain,
      ...
      toast({
        title: "Domain added",
        description:
          "Your domain has been successfully added to the project and saved to your account.",
      });
    }
    ...
    <div className="space-y-6">
      ...
      {plan === "starter" ? (
        <Card className="border-amber-200 bg-amber-50 dark:bg-amber-950/20 dark:border-amber-800">
          ...
          <CardContent>
            <p className="text-amber-700 dark:text-amber-300">
              Custom domains are available on Pro and Enterprise plans. Upgrade
              to connect your own domain name to your websites.
            </p>
          </CardContent>
          <CardFooter>
            <Button
              ...
              <CardHeader className="pb-2">
                <div className="flex items-center justify-between">
                  <div className="space-y-1">
                    ...
                    <CardDescription>
                      Connected to {domain.websiteName} • Added on{" "}
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-muted-foreground">
              Add a custom domain to make your websites accessible at your own
              domain names.
            </p>
          </CardContent>
          <CardFooter>
