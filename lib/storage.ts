    ...
    for (const bucket of Object.values(StorageBucket)) {
      ...
      if (!existingBucket) {
        const { error } = await supabase.storage.createBucket(bucket, {
          ...
          fileSizeLimit: bucket === StorageBucket.WEBSITE_ASSETS ? 10485760 : 5242880, // 10MB for website assets, 5MB for others
        })
...
export async function listFiles(
  ...
  try {
    ...
    return handleError(error, `Failed to list files in ${bucket}/${path || ""}`)
  }
}
...
export async function moveFile(bucket: StorageBucket, fromPath: string, toPath: string): Promise<void> {
  try {
    ...
    return handleError(error, `Failed to move file from ${bucket}/${fromPath} to ${bucket}/${toPath}`)
  }
}
...
export async function copyFile(bucket: StorageBucket, fromPath: string, toPath: string): Promise<void> {
  try {
    ...
    return handleError(error, `Failed to copy file from ${bucket}/${fromPath} to ${bucket}/${toPath}`)
  }
}

// Get metadata for a file
export async function getFileMetadata(bucket: StorageBucket, path: string): Promise<any> {
  try {
    const supabase = getSupabaseClient()
...
export async function uploadImage(
  ...
  path: string,
  file: File | Blob,
  options?: { contentType?: string; upsert?: boolean; generateThumbnail?: boolean },
