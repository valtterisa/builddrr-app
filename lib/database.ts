...
export async function upsertProfile(profile: Partial<Profile> & { id: string }): Promise<Profile> {
  try {
    ...
    return handleError(error, `Failed to upsert profile for user ${profile.id}`)
  }
}
...
export async function createWebsite(userId: string, websiteData: Partial<Website>): Promise<Website> {
  try {
    const supabase = getSupabaseClient()
    const { data, error } = await supabase
      .from("websites")
...
export async function updateWebsite(websiteId: string, updates: Partial<Website>): Promise<Website> {
  try {
    const supabase = getSupabaseClient()
    const { data, error } = await supabase.from("websites").update(updates).eq("id", websiteId).select().single()
...
export async function incrementWebsiteVisits(websiteId: string): Promise<void> {
  try {
    const supabase = getSupabaseClient()
    const { error } = await supabase.rpc("increment_website_visits", { website_id: websiteId })

    if (error) throw error
  } catch (error) {
    console.error(`Failed to increment visits for website ${websiteId}:`, error)
...
export async function createDomain(websiteId: string, domainData: Partial<Domain>): Promise<Domain> {
  try {
    const supabase = getSupabaseClient()
    const { data, error } = await supabase
      .from("domains")
    ...
    return handleError(error, `Failed to create domain for website ${websiteId}`)
  }
}

export async function updateDomain(domainId: string, updates: Partial<Domain>): Promise<Domain> {
  try {
    const supabase = getSupabaseClient()
    const { data, error } = await supabase.from("domains").update(updates).eq("id", domainId).select().single()
...
// Integrations
export async function getIntegrations(userId: string, websiteId?: string): Promise<Integration[]> {
  try {
    const supabase = getSupabaseClient()
...
export async function createIntegration(userId: string, integrationData: Partial<Integration>): Promise<Integration> {
  try {
    const supabase = getSupabaseClient()
    const { data, error } = await supabase
      .from("integrations")
    ...
    return handleError(error, `Failed to create integration for user ${userId}`)
  }
}

export async function updateIntegration(integrationId: string, updates: Partial<Integration>): Promise<Integration> {
  try {
    const supabase = getSupabaseClient()
    const { data, error } = await supabase
      .from("integrations")
...
export async function createAsset(websiteId: string, assetData: Partial<Asset>): Promise<Asset> {
  try {
    const supabase = getSupabaseClient()
    const { data, error } = await supabase
      .from("assets")
    ...
    return handleError(error, `Failed to create asset for website ${websiteId}`)
  }
}
